...
from lib.PDE import BSMModel, CallOption
...
class BSMThetaScheme:
    def __init__(self, model, payoff, T, M, N, theta):
        ...
        self.x_max = model.x_max(self.T)  # model 模型
        ...
        sigma_square = self.model.sigma*self.model.sigma # sigm--∑
        r = self.model.r
        self.theta = theta  # theta--θ
...
for xStep in xSteps:
    ...
    shceme = BSMThetaScheme(model, payoff, 5.0, timeStep, xStep, 0.0)
    scheme.roll_back()
    interp = CubicNaturalSpline(np.exp(scheme.xArray), scheme.C[:,-1])
    price = interp(100.0)
...
pylab.legend([u'Crank - Nicolson差分格式', u'隐式差分格式', u'解析解（欧式）'], prop = font)
